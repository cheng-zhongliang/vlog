...
void vlog_valist(enum vlog_module module, enum vlog_level level, const char* message, va_list args) {
...
...) {
    ...
    if(rl->n_dropped) {
        vlog(module, level, "Dropped %u messages in last %u seconds due to excessive rate",
        rl->n_dropped, (unsigned int)(time(NULL) - rl->first_dropped));
        rl->n_dropped = 0;
    }
}